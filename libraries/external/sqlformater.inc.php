...
class SqlFormatter
{
  ...
  protected static $reserved_toplevel = array(
    ...
    'VALUES', 'UPDATE', 'HAVING', 'ADD', 'AFTER', 'ALTER TABLE', 'DELETE FROM', 'UNION ALL', 'UNION', 'EXCEPT', 'INTERSECT', 'BEGIN', 'END'
  );
  ...
  protected static $functions = array(
    ...
    'RELATED', 'RELEASE_LOCK', 'REPEAT', 'REPLACE', 'REVERSE', 'RIGHT', 'ROUND', 'ROW_COUNT', 'ROW_NUMBER', 'RPAD', 'RTRIM', 'SCHEMA', 'SECOND', 'SEC_TO_TIME',
  ...
  protected static function getNextToken($string, $previous = NULL)
  {
    ...
    if (preg_match('/^\s+/', $string, $matches))
    {
      return array(
        'value' => $matches[0],
        'type' => self::TOKEN_TYPE_WHITESPACE
      );
    }
    ...
    {
      $return = array(
        'type' => (($string[0] === '`' || $string[0] === '[') ? self::TOKEN_TYPE_BACKTICK_QUOTE : self::TOKEN_TYPE_QUOTE),
        'value' => self::getQuotedString($string)
      );
    ...
    if (($string[0] === '@' || $string[0] === ':') && isset($string[1]))
    {
      ...
      {
        $ret['value'] = $string[0] . self::getQuotedString(substr($string, 1));
      }
    ...
    {
      return array(
        'type' => self::TOKEN_TYPE_FUNCTION,
        'value' => substr($string, 0, strlen($matches[1]) - 1)
      );
    }

    // A reserved word cannot be preceded by a '.'
    // this makes it so in "mytable.from", "from" is not considered a reserved word
    if (!$previous || !isset($previous['value']) || $previous['value'] !== '.')
    {
      $upper = strtoupper($string);
      ...
      {
        return array(
          'type' => self::TOKEN_TYPE_RESERVED_TOPLEVEL,
          'value' => substr($string, 0, strlen($matches[1]))
        );
      }
      ...
      {
        return array(
          'type' => self::TOKEN_TYPE_RESERVED_NEWLINE,
          'value' => substr($string, 0, strlen($matches[1]))
        );
      }
   ...
   */
  public static function format($string, $highlight = TRUE, $values = FALSE)
  {
    ...
    {
      if ($token['type'] !== self::TOKEN_TYPE_WHITESPACE)
      {
        $token['i'] = $i;
        $tokens[] = $token;
      }
    ...
    foreach ($tokens as $i => $token)
    {
      ...
      if ($token['type'] === self::TOKEN_TYPE_COMMENT || $token['type'] === self::TOKEN_TYPE_BLOCK_COMMENT)
      {
        if ($token['type'] === self::TOKEN_TYPE_BLOCK_COMMENT)
        {
      ...
      if ($inline_parentheses > 0)
      {
//        echo 'inline: ' . $token['value'] . '<br>';
...
//        }
//
//        $inline_count += strlen($token['value']);
      }
...
//          }
          ...
          if ($next['type'] === self::TOKEN_TYPE_RESERVED_TOPLEVEL ||
              $next['type'] === self::TOKEN_TYPE_RESERVED_NEWLINE ||
              $next['type'] === self::TOKEN_TYPE_COMMENT ||
              $next['type'] === self::TOKEN_TYPE_BLOCK_COMMENT)
          {
            break;
          }
        ...
        if (isset($original_tokens[$token['i'] - 1]) && $original_tokens[$token['i'] - 1]['type'] !== self::TOKEN_TYPE_WHITESPACE)
        {
          $return = rtrim($return, ' ');
        }
      ...
      elseif ($token['value'] === ')')
      {
        ...
        if ($indent_level < 0)
        {
          ...
          if ($highlight)
          {
            $return .= "\n" . self::highlightError($token['value']);
            continue;
          }
        }
      ...
      elseif ($token['type'] === self::TOKEN_TYPE_RESERVED_TOPLEVEL)
      {
        ...
        if (strpos($token['value'], ' ') !== FALSE || strpos($token['value'], "\n") !== FALSE || strpos($token['value'], "\t") !== FALSE)
        {
          $highlighted = preg_replace('/\s+/', ' ', $highlighted);
        }
        //if SQL 'LIMIT' clause, start variable to reset newline
        if ($token['value'] === 'LIMIT' && $inline_parentheses <= 0)
        {
          $clause_limit = TRUE;
        }
      }
      ...
      elseif ($clause_limit && $token['value'] !== "," && $token['type'] !== self::TOKEN_TYPE_NUMBER && $token['type'] !== self::TOKEN_TYPE_WHITESPACE)
      {
        $clause_limit = FALSE;
      }

      // Commas start a new line (unless within inline parentheses or SQL 'LIMIT' clause)
      elseif ($token['value'] === ',' && $inline_parentheses <= 0)
      {
      ...
      elseif ($token['type'] === self::TOKEN_TYPE_RESERVED_NEWLINE)
      {
      ...
      elseif ($token['type'] === self::TOKEN_TYPE_BOUNDARY)
      {
        if (isset($tokens[$i - 1]) && $tokens[$i - 1]['type'] === self::TOKEN_TYPE_BOUNDARY)
        {
          if (isset($original_tokens[$token['i'] - 1]) && $original_tokens[$token['i'] - 1]['type'] !== self::TOKEN_TYPE_WHITESPACE)
          {
            $return = rtrim($return, ' ');
      ...
      elseif ($token['type'] === self::TOKEN_TYPE_VARIABLE)
      {
        if ($values)
        {
      ...
      // Asthetic preference.
      if (in_array($token['value'], array('FROM', 'WHERE', 'ORDER BY')))
      {
        $newline = FALSE;
      }
      ...
      if ($token['value'] === '-' && isset($tokens[$i + 1]) && $tokens[$i + 1]['type'] === self::TOKEN_TYPE_NUMBER && isset($tokens[$i - 1]))
      {
  ...
  public static function splitQuery($string)
  {
    ...
    foreach ($tokens as $token)
    {
      ...
      if ($token['type'] !== self::TOKEN_TYPE_WHITESPACE && $token['type'] !== self::TOKEN_TYPE_COMMENT && $token['type'] !== self::TOKEN_TYPE_BLOCK_COMMENT)
      {
        $empty = FALSE;
      }

      $current_query .= $token['value'];
    }
  ...
  public static function compress($string)
  {
    ...
    foreach ($tokens as $token)
    {
      ...
      elseif ($token['type'] === self::TOKEN_TYPE_RESERVED || $token['type'] === self::TOKEN_TYPE_RESERVED_NEWLINE || $token['type'] === self::TOKEN_TYPE_RESERVED_TOPLEVEL)
      {
  ...
  protected static function highlightToken($token)
  {
    ...
    else
    {
      ...
      {
        $token = htmlentities($token['value'], ENT_COMPAT | ENT_IGNORE, 'UTF-8');
      }
      else
      {
        $token = htmlentities($token['value'], ENT_COMPAT, 'UTF-8');
      }
    }
